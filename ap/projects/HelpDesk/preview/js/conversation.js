// const CnvsnPnl = (function () {
  ...
  function getResponse(responses, gen) {
    ...
    switch (gen.response_type) {
      ...
      case 'option':
        let list = getOptions(gen.button);
        responses.push({
          type: gen.response_type,
          innerhtml: title + list
        });
        sessionStorage.setItem('buttons', JSON.stringify(gen.button));
        break;
  ...
  function getOptions(optionsList) {
    ...
    if (optionsList != null) {
        optionsList.forEach(function (element) {
            let id = element.btnType;
            if (element.btnType == 'nextto') {
                let item = '<div class="options-button" id="' + id + '" onclick = "CnvsnPnl.sendMessage(\'' + element.btnText + '\',\'button\',\'' + element.btnValue + '\'); CnvsnPnl.toChangeButtonColor(this, id)">' +  element.btnLabel + '</div>'
                list += item;
            } else if (element.btnType == 'url') {
                let item = '<a class="options-button" id="' + id + '" href = "' + element.btnValue + '"; target = "_blank"; rel = "noreferrer noopener"; onclick = "CnvsnPnl.sendMessage(\'[' + element.btnText + ']\',\'button\'); CnvsnPnl.clickDisplay(\'url\'); CnvsnPnl.toChangeButtonColor(this, id); ">' + element.btnLabel + '</a>'
                list += item;
            }
            else if (element.btnType == 'download') {
                let item = '<a class="options-button" id="' + id + '" href = "'+element.btnValue+'"onclick = "CnvsnPnl.sendMessage(\'[' + element.btnText + ']\',\'button\'); CnvsnPnl.clickDisplay(\'download\'); CnvsnPnl.toChangeButtonColor(this, id); " target="_blank" download>' + element.btnLabel + '</a>'
                list += item;
            }
            else if (element.btnType == 'mailto') {
                let item = '<a class="options-button" id="' + id + '" onclick = "CnvsnPnl.sendMessage(\'[' + element.btnText + ']\',\'button\'); CnvsnPnl.clickDisplay(\'mailto\'); CnvsnPnl.toChangeButtonColor(this, id)" href="mailto: ' + element.btnValue.mailName + '?subject=' + element.btnValue.mailTitle + '&body=' + element.btnValue.mailBody + '">' + element.btnLabel + '</a>'
                list += item;
            }
        })
    }
  ...
  // 點選button後更改顏色
  function toChangeButtonColor(box,id){
    ...
    if(box.parentNode.parentNode.classList[0]=="welcome"){
      ...
      box.style.background="url('./employee/img/Q2.png')";
    ...
    if(box.parentNode.parentNode.classList[0]=="blackBox-inner"){
      ...
      /*寄信button，會更改icon*/
      if(id == "寄信"){
        box.style.background='url("./img/寄信_e60012_icon.png")';
        box.style.backgroundPosition='bottom 5px right 4px';
        box.style.backgroundRepeat="no-repeat";
        box.style.backgroundColor="#D0D0D0";
        box.style.color="#E60012";
      }else{
        box.style.background='url("./img/下載_e60012_icon.png")';
        box.style.backgroundPosition='bottom 5px right 4px';
        box.style.backgroundRepeat="no-repeat";
        box.style.backgroundColor="#D0D0D0";
      }
  ...
  function getslider(sliders) {
    ...
    if (sliders) {
      ...
      items += '</div></div><button class="left" onclick="CnvsnPnl.left(this)"><i class="fa fa-angle-left"></i></button><button class="right" onclick="CnvsnPnl.right(this)"><i class="fa fa-angle-right"></i></button>'
    }
    return items;
  };
  ...
  function getButtons(buttons) {
    ...
    if (buttons) {
      buttons.forEach(function (element) {
        let id = '';
        if (element.btnType == "nextto") {
          let item = '<a onclick="CnvsnPnl.sendMessage(\'' + element.btnText + '\',\'button\',\'' + element.btnValue + '\'); CnvsnPnl.toChangeButtonColor(this,id)">' + element.btnLabel + '</a>';
          btn += item;
        } else if (element.btnType == "url") {
          let item = '<a href="' + element.btnValue + '"; target="_blank"; rel="noreferrer noopener"; onclick="CnvsnPnl.sendMessage(\'' + element.btnText + '\',\'button\'); CnvsnPnl.clickDisplay(\'url\');">' + element.btnLabel + '</a>';
          btn += item;
        } else if (element.btnType == "download") {   // CnvsnPnl.sendMessage(\'' + element.btnText + '\',\'button\');
          let item = '<a onclick="CnvsnPnl.clickDisplay(\'download\'); Api.downloadButton(\'' + element.btnValue + '\');">' + element.btnLabel + '</a>';
          btn += item;
        } else if (element.btnType == "mailto") {
          let item = '<a onclick="CnvsnPnl.sendMessage(\'' + element.btnText + '\',\'button\'); CnvsnPnl.clickDisplay(\'mailto\'); " href="mailto:' + element.btnValue.mailName + '?subject=' + element.btnValue.mailTitle + '&body=' + element.btnValue.mailBody + '">' + element.btnLabel + '</a>';
          btn += item;
        }
      });
    }
