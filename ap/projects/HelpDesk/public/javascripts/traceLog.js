...
var traceLogController = function () {
  ...
  function __init__() {
    ...
    $download.on('click', function (e) {
      var header = $(traceLogDatatable.table().header()).find('th').toArray().map(function (th) {
        return $(th).text();
      });
      var content = traceLogDatatable.rows().data().toArray();
      DownloadGreatArray([header].concat(content), '使用歷程'); // let header = traceLogDatatable.columns().header().toArray().map(th => $(th).text())
