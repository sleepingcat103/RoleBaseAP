...
function callBackendAPI(path, method, body, headers) {
    return callBackend('/backendApi' + path, method, body, headers)
    .then(res => {
    ...
    return new Promise(function(resolve, reject) {
        ...
        $.ajax({
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                reject({ 
                    status: jqXHR.status,
                    desc: jqXHR.responseJSON || { msg: textStatus + ' ' + errorThrown }
                });
            }
        });
    });
...
function callBackendAPI_Multipart(path, method, formData, headers) {
    return callBackend_Multipart('/backendApi' + path, method, formData, headers)
    .then(res => {
        console.log('[success]', path, res);
        return res;
    })
...
function secondsToTime(ss) {
    let hh = Math.floor(ss/3600); //Get whole hours
    ss -= hh*3600;
    let mm = Math.floor(ss/60); //Get remaining minutes
    ss -= mm*60;
    return (hh ? hh + " 小時 " : "") + 
            (mm ? mm + " 分鐘 " : "") + 
            (ss ? ss : 0) + " 秒"; 
...
function tempMask(message, during) {
    ...
    $('body').append('<div class="sky active" id="'+id+'" onclick="$(this).remove()">' + 
        (message ? 
...
function stableMask(message) {
    ...
    $('body').append('<div class="sky active" id="'+id+'">' +
        (message ? 
        '<div class="sky-content">' + 
            message +
        '</div>' : '') + ' \
...
function DownloadGreatCSV($selector, filename) {
	...
	$selector.find('tr').toArray().forEach(function(tr){
		...
		$(tr).children('td,th').toArray().forEach(function(cell){
			let mycontent = $(cell).html().includes('<img ') ? '圖片: ' + $($(cell).html()).prop('src') :  
				$(cell).text().replace(/\n/g," ").replace(/,/g,"，");
			row.push(mycontent)
		})
		myCSV.push(row.join(','));
...
function sidebar($selector, appendClass, dismiss) {
    let sidebarId = 'sidebar-' + uuid();
    dismiss = dismiss || false;

    $selector.after(`<div class="sidebar ${ appendClass ? appendClass : ''}" id="${ sidebarId }">
        <div class="sky"></div>
    </div>`);
...
function getFormDataJson($form) {
    ...
    let result = $form.serializeArray().reduce(function(obj, item) {
...
let notify = (() => {
    ...
    let defaultSettings = { 
        ...
        // {0} = type {1} = title {2} = message {3} = url {4} = target
        // template: `
    ...
    let doNotify = (type, msg, settings) => {
        if(!type || !msg) return;

        // full options: { icon, title, message, url, target }
        let options;
        if(typeof(msg) === 'object' && (msg.message || msg.title)) {
            options = msg;
